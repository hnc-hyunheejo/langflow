...
class OpenDataLoaderPDFComponent(BaseFileComponent):
    ...
    def _get_content_safety_off(self) -> list[str] | None:
        ...
        if invalid:
            self.log(f"Warning: unknown content safety filter(s): {invalid}. "
            ...
            for file_path in file_paths:
                try:
                    ...
                    if self.hybrid != "off":
                        ...
                        if self.hybrid_timeout is not None:
                    ...
                    if expected_output.exists():
                        ...
                        output_files = sorted(output_path.glob(f"*.{ext}"), key=lambda p: p.stat().st_mtime, reverse=True)
